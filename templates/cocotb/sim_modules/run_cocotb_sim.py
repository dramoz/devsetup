...
def run_cocotb_sim(
  ...
  hdl_sources: Optional[Union[src_path_t, Sequence[src_path_t]]] = None,
  ...
  :param simulator_engine: simulator to use (Verilator, Icarus, Modelsim/Questa, ...) - if None and not environmental SIM variable is found, defaults to verilator
  ...
  cocotb_log_long_format = os.environ.get('COCOTB_REDUCED_LOG_FMT', '0')
  # -------------------------------------------------------------------
  # Check arguments
  ...
  if hdl_sources is None:
    ...
    if isinstance(hdl_sources, Path):
      ...
      hdl_sources = Path(hdl_sources).read_text().split("\n")
    ...
    verilog_sources = [file for file in hdl_sources if Path(file).suffix[1:] in verilog_ext]
    vhdl_sources = [file for file in hdl_sources if Path(file).suffix[1:] in vhdl_ext]
  ...
  sim_cocotbtest = ["icarus", "questa", "ius", "xcelium", "vcs", "ghdl", "riviera", "verilator"]
  ...
  if sim not in sim_cocotbtest:
    raise ValueError("Selected simulator engine ({sim}) is not supported by cocotb-test")
  ...
  extra_env['COCOTB_REDUCED_LOG_FMT'] = cocotb_log_long_format
  # -------------------------------------------------------------------
  ...
  if sim == 'verilator':
    ...
    # Include(s) directories, path from sim_build/test_id/
    compile_args = [f"+incdir+{d}" for d in include_dirs]
